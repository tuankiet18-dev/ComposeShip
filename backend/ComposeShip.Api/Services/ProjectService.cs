...
public class ProjectService
{
    ...
    private static ComposeFileAssessment AnalyzeComposeFile(string path, string yamlContent)
    {
        ...
        try
        {
            ...
            foreach (var serviceEntry in servicesNode.Children)
            {
                ...
                {
                    isDevelopment = true;
                    errors.Add($"Service '{serviceName}' mounts repository files from the host. Local development mounts cannot run after ComposeShip cleans its temporary workspace.");
                }
    ...
    private bool IsPostStartCommandsEnabled() =>
        _configuration.GetValue("ComposeShip:EnablePostStartCommands", false)
        || _configuration.GetValue("COMPOSESHIP_ENABLE_POST_START_COMMANDS", false);
    ...
    private DeploymentGraphResponse BuildConfiguredDeploymentGraph(Project project, string branch, string? subfolder, string? composeFile)
    {
        ...
        {
            edges.Add(new DeploymentGraphEdgeResponse(
                $"connects_to:service:{serviceName}:network:composeship-runtime",
                "connects_to",
                $"service:{serviceName}",
                "network:composeship-runtime",
                "composeship runtime",
