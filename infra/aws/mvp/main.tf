...
resource "random_password" "invite_code_pepper" {
  length  = 48
  special = false
}
...
resource "aws_security_group" "control_plane" {
  ...
  ingress {
    description     = "CloudFront-only HTTP origin traffic"
    from_port       = 80
    to_port         = 80
    protocol        = "tcp"
  ...
  ingress {
    description = "Execution node agent to private control-plane API"
    from_port   = 5000
    to_port     = 5000
    protocol    = "tcp"
...
resource "aws_iam_role" "execution_node" {
  name = "${var.project_name}-execution-node-role"
...
resource "aws_iam_role_policy_attachment" "execution_node_ssm" {
  role       = aws_iam_role.execution_node.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}
...
resource "aws_iam_instance_profile" "execution_node" {
  name = "${var.project_name}-execution-node-profile"
  role = aws_iam_role.execution_node.name
}
...
resource "aws_instance" "control_plane" {
  ...
  # PostgreSQL lives on this node's encrypted root volume. Application source
  # changes are rolled out by scripts/rollout-control-plane.sh, never by
  # replacing this instance through Terraform user-data changes.
  ...
  root_block_device {
    ...
    # The MVP PostgreSQL volume is still host-local. Preserve it if an
    # operator terminates/replaces the instance so recovery is deliberate,
    # not an accidental data-loss event.
...
resource "aws_launch_template" "execution_node" {
  ...
  iam_instance_profile {
    name = aws_iam_instance_profile.execution_node.name
  }
