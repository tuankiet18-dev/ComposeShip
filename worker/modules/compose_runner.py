...
NGINX_CAPABILITIES = ["CHOWN", "SETUID", "SETGID", "NET_BIND_SERVICE"]
...
def _platform_capabilities(service_name: str, service: dict[str, Any]) -> list[str]:
    ...
    # The official nginx entrypoint starts as root, changes ownership of its
    # cache directories, then workers drop to the nginx UID/GID. Keep only
    # those setup privileges scoped to conventional frontend/proxy services
    # (or an explicit nginx image), while application services retain the
    # platform's default least-privilege profile.
